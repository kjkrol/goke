...
func (v *View{{$n}}[{{types $n}}]) All() iter.Seq2[Entity, Row{{$n}}[{{types $n}}]] {
    return func(yield func(Entity, Row{{$n}}[{{types $n}}]) bool) {
       for _, arch := range v.matched {
          ...
          {{range $i := seq $n}}
          idx{{add $i 1}} := arch.colTable[v.ids[{{$i}}]]
          col{{add $i 1}} := &arch.columns[idx{{add $i 1}}] 
    ...
    return func(yield func(Entity, Row{{$n}}[{{types $n}}]) bool) {
       ...
       for _, e := range entities {
          ...
          if arch != lastArch {
             ...
             c{{add $i 1}} = &arch.columns[arch.colTable[v.ids[{{$i}}]]]
             {{end}}
             lastArch = arch
          }
...
func main() {
	...
	// Używamy samej nazwy pliku, aby uniknąć problemu ze ścieżką przy go generate
