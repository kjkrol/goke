...
//
//   - Base      — raw pointer to the start of the current chunk's data allocation
...
//
// Idx is assigned when the column is registered for tracking; re-registering
// overwrites it, so the last registration wins. A Col[T] that has never been
// registered must not be used.
package iter
