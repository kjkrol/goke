...
func main() {
	...
	diceType := goke.ComponentRegister[Dice](engine)
	playerType := goke.ComponentRegister[Player](engine)

	// 2. Setup Entities & Components
	diceEnt := goke.EntityCreate(engine)
	if ptr, _ := goke.EntityEnsureComponent[Dice](engine, diceEnt, diceType); ptr != nil {
		*ptr = Dice{Value: 0}
	}

	// Setup player entities
	p1 := goke.EntityCreate(engine)
	if ptr, _ := goke.EntityEnsureComponent[Player](engine, p1, playerType); ptr != nil {
		*ptr = Player{Bet: 0}
	}

	p2 := goke.EntityCreate(engine)
	if ptr, _ := goke.EntityEnsureComponent[Player](engine, p2, playerType); ptr != nil {
		*ptr = Player{Bet: 0}
	}
	...
	rollSys := goke.SystemFuncRegister(engine, func(cb *goke.Schedule, d time.Duration) {
	...
	betSys := goke.SystemFuncRegister(engine, func(cb *goke.Schedule, d time.Duration) {
	...
	judgeSys := goke.SystemFuncRegister(engine, func(schedule *goke.Schedule, d time.Duration) {
		if gameFinished {
			return
		}
		turnCounter++

		dice, _ := goke.EntityGetComponent[Dice](engine, diceEnt, diceType)
	...
	displayWinnerSys := goke.SystemFuncRegister(engine, func(cb *goke.Schedule, d time.Duration) {
