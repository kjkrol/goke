...
// subset/order dimensions, but through ValueMigrator + CmdBufMassMigrateInit
// instead of Migrator + CmdBufMassMigrate — isolating the payload
// mechanism's overhead (arena reservation, per-run copyMemory, slow-path
// origIdx compaction) from everything else, since both leaves add exactly
// one Pos component onto the same Base-anchored world.
func Benchmark_ValueMigrator_Add(b *testing.B) {
	...
	for _, subset := range []int{filterSubsetSize, entitiesNumber} {
		runValueMigratorLeaf(b, ecs,
			...
			subset,
			func() (*goke.Query, *goke.ValueMigrator, *goke.Comp[Pos], goke.System) {
				populateBase(ecs, entitiesNumber)
				var col goke.Comp[Pos]
				fwd := ecs.NewValueMigratorBuilder(&col).Build()
				rev := ecs.NewMigratorBuilder().Delete(goke.Del[Pos]()).Build()
