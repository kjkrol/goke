...
### Added ✨
* **`Factory`/`NewFactory` now accepts zero-size tag components mixed into the `Add` list** - pass `&Comp[T]{}` for a tag alongside real data components; it marks the spawned archetype without occupying a data column. Previously `NewFactory` panicked on any zero-size type ("tags are not allowed as data columns"), even though `Editor`/`OptComp` already supported them via `Include`/`Optional`.
...
### Added ✨
* **`Optional[T]()`** (`QueryBuilder.Optional` + `OptComp[T]`) — track a data component that reads its value when the matched archetype carries it, without requiring or excluding it from the match. Replaces the old two-query (`Include`+`Exclude`) workaround for reading an optional component with one pass, ~14% faster; works across `All`/`Pick`/`Seek`/`SeekH` with zero extra allocation. See `BENCHMARKS.md`.
