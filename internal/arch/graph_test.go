...
func TestGraph_LinkNextArch(t *testing.T) {
	...
	root := &ar.Archetypes[RootID]
	ar.AddEntity(uid.UID64(1), RootID)
	ar.UpsertComp(uid.UID64(1), posInfo)
	...
	posArch := &ar.Archetypes[posArchID]
...
func TestGraph_MultipleNextEdges(t *testing.T) {
	...
	ar.AddEntity(uid.UID64(1), RootID)
	ar.AddEntity(uid.UID64(2), RootID)
	ar.UpsertComp(uid.UID64(1), posInfo)
	ar.UpsertComp(uid.UID64(2), velInfo)
