...
func TestTable_RemoveAt_LastSlotNoSwap(t *testing.T) {
	...
	ptr := tbl.ChunkPtrAt(pos.Idx)

	*(*int64)(tbl.ComponentAt(ptr, pos.Slot, 1)) = 77

	movedID, swapped := tbl.RemoveAt(ptr, pos.Slot)
	...
	if got := *(*int64)(tbl.ComponentAt(ptr, pos.Slot, 1)); got != 0 {
