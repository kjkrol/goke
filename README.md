...
| [`internal/comp`](https://github.com/kjkrol/goke/tree/main/internal/comp) | Shared component primitives used across all internal packages — type registration, metadata, and blueprint definitions |
| [`internal/soa`](https://github.com/kjkrol/goke/tree/main/internal/soa) | Low-level Structure-of-Arrays memory layout — L1-cache-sized fixed slabs, column offset calculation, position tracking within a growing slab collection |
| [`internal/colstore`](https://github.com/kjkrol/goke/tree/main/internal/colstore) | Column-oriented storage for a single archetype — manages component columns over SoA slabs, resolves component IDs to memory locations in O(1) |
| [`internal/arch`](https://github.com/kjkrol/goke/tree/main/internal/arch) | Archetype identity, archetype graph, and entity lifecycle — tracks which archetype each entity belongs to and caches structural transitions as graph edges |
| [`internal/query`](https://github.com/kjkrol/goke/tree/main/internal/query) | Query layer: bakes component masks into precomputed per-archetype offsets, enabling zero-allocation iteration and O(1) per-entity lookup |
| [`internal/orch`](https://github.com/kjkrol/goke/tree/main/internal/orch) | Plan-based task orchestrator: sequential/parallel execution, deferred mutations via command buffers |
| [`internal/reg`](https://github.com/kjkrol/goke/tree/main/internal/reg) | Top-level world registry — wires together all subsystems and exposes the unified API for entity and component management |
